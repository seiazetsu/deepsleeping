...
import {
  ...
  updateOnsenLog,
  type OnsenLogInput,
} from "@/lib/onsenLogRepository";
import { getFirebaseStorage } from "@/lib/firebase";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import type { OnsenLog } from "@/types/onsenLog";

export default function EditOnsenLogPage() {
  const router = useRouter();
  const params = useParams();
  const id = params?.id as string | undefined;

  const [log, setLog] = useState<OnsenLog | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getOnsenLogById(id);
        if (!data) {
          setLoadingError("ログが見つかりませんでした。");
          return;
        }
        setLog(data);
        setDate(data.date);
        setOnsenName(data.onsenName);
        setSleepScore(String(data.sleepScore ?? ""));
        setMemo(data.memo ?? "");
        setRating(data.rating ?? 3);
        setPreviewUrl(data.photoUrl ?? "");
      } catch (err) {
        console.error("[edit] load error", err);
        setLoadingError("ログの読み込みに失敗しました。");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. 温泉名が変わった場合はジオコードを取り直す
      let lat: number | undefined = log?.lat;
      let lng: number | undefined = log?.lng;

      if (onsenName.trim() && onsenName.trim() !== log?.onsenName) {
        try {
          ...
          console.warn("[edit] geocode error", geoErr);
        }
      }

      // 2. 画像が新しく選ばれていたらアップロード
      let uploadedPhotoUrl: string | undefined = log?.photoUrl;
      ...
      if (imageFile) {
        try {
          ...
          console.warn("[edit] image upload failed", uploadErr);
          // 写真だけ失敗しても他は更新する
        }
      }

      // 3. Firestore 更新
      const updateData: Partial<OnsenLogInput> = {
        date,
      ...
      await updateOnsenLog(id, updateData);
  ...
  if (loadingError || !log) {
    return (
      ...
      >
        ...
        <p>{loadingError ?? "ログが見つかりませんでした。"}</p>
      </main>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <label style={labelStyle}>
            <span style={labelTextStyle}>写真（任意・変更したい場合）</span>
            <input
              type="file"
              accept="image/*"
              style={inputStyle}
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  setImageFile(null);
                  // 既存写真に戻したい場合は log.photoUrl をセット
                  setPreviewUrl(log.photoUrl ?? "");
                }
              }}
            />
            <span
            ...
            >
              新しい写真を選ぶと、アップロード時に幅600pxに縮小されます。
            </span>
          </label>
